...
import { 
  ...
  ChevronDown, ChevronUp, Ticket, Mail, Globe, Smartphone, UploadCloud, Clock, Search, X,
  Eye, Trash2, Building, Banknote, Landmark, Pencil, Copy, Coffee, Luggage, PlusCircle
} from 'lucide-react';
...
const BAGGAGE_OPTIONS = [
  { id: 'none', label: 'No Extra Baggage', desc: 'Standard allowance applies', price: 0 },
  { id: '5kg', label: '+ 5 KG Extra', desc: 'Ideal for an extra small bag', price: 4500 },
  { id: '10kg', label: '+ 10 KG Extra', desc: 'Perfect for shopping haul', price: 8000 },
  { id: '20kg', label: '+ 20 KG Extra', desc: 'Full additional suitcase', price: 15000 }
];

const MEAL_OPTIONS = [
  { id: 'none', label: 'Standard Meal', desc: 'Default in-flight meal', price: 0 },
  { id: 'veg', label: 'Vegetarian Meal', desc: 'Strictly plant-based', price: 0 },
  { id: 'chicken', label: 'Chicken Biryani', desc: 'Traditional subcontinental', price: 2500 },
  { id: 'premium', label: 'Premium Steak', desc: 'Gourmet dining experience', price: 5000 }
];
    ...
    <div className="relative flex-1 flex flex-col">
       <div className={`relative w-full h-11 bg-white border ${isActive ? 'border-[#1a4484] ring-1 ring-[#1a4484]' : 'border-slate-300 hover:border-[#1a4484]'} rounded-lg flex items-center overflow-hidden transition-all`}>
         ...
         <div className="absolute right-1 sm:right-2 flex items-center gap-1 bg-white pointer-events-none shrink-0">
            {isActive && <Search size={14} className="text-slate-300" />}
            {!isActive && !value && <ChevronDown size={14} className="text-slate-400 shrink-0" />}
            {value && <CheckCircle size={14} className="text-emerald-500 shrink-0" />}
         </div>
       </div>
...
const Stepper = ({ currentStep, onBack, bookingType }) => {
  ...
  const steps = bookingType === 'ticket' ? [
    { id: 1, name: 'Travelers', icon: Users },
    { id: 2, name: 'Extras', icon: PlusCircle },
    { id: 3, name: 'Review', icon: FileText },
    { id: 4, name: 'Payment', icon: CreditCard },
    { id: 5, name: lastStepName, icon: CheckCircle }
  ] : [
    { id: 1, name: 'Travelers', icon: Users },
...
export const BookingView = ({
  ...
  // Define step indices dynamically based on booking type
  const maxSteps = bookingType === 'ticket' ? 5 : 4;
  const successStepIndex = bookingType === 'ticket' ? 5 : 4;
  const paymentStepIndex = bookingType === 'ticket' ? 4 : 3;
  const reviewStepIndex = bookingType === 'ticket' ? 3 : 2;
  const extrasStepIndex = bookingType === 'ticket' ? 2 : -1;
  ...
  const [activeExtraTab, setActiveExtraTab] = useState('baggage'); 
  const [extras, setExtras] = useState({
    seat: { id: 'none', label: 'Not selected', price: 0 },
    baggage: BAGGAGE_OPTIONS[0],
    meal: MEAL_OPTIONS[0]
  });

  const extrasTotal = bookingType === 'ticket' ? (extras.seat.price + extras.baggage.price + extras.meal.price) : 0;
  ...
  const paymentMethodFee = paymentMethod === 'card' ? Math.floor((dynamicTotal + extrasTotal + bookingFee) * 0.02) : 0; 
  const baseFare = Math.floor(dynamicTotal * 0.85);
  const taxes = dynamicTotal - baseFare;
  const grandTotal = dynamicTotal + extrasTotal + bookingFee + paymentMethodFee;
  ...
  const PRICE_DETAILS = {
    ...
    extras: extrasTotal,
    total: grandTotal,
    refundableAmount: Math.floor(baseFare * 0.8) + taxes,
    nonRefundableAmount: Math.floor(baseFare * 0.2) + bookingFee + paymentMethodFee + extrasTotal,
  ...
  const [bookingTimer, setBookingTimer] = useState(15 * 60); 
  const [paymentTimer, setPaymentTimer] = useState(60 * 60 + 52 * 60 + 42); 
    ...
    for(let r=1; r<=22; r++) {
        ...
        ['A','B','C','D','E','F'].forEach(col => {
            ...
            row.seats.push({
                ...
                col: col,
                isOccupied,
                price: r <= 3 ? 4500 : (r === 12 || r === 13 ? 3500 : 1500)
            });
        });
        rows.push(row);
    }
  ...
  const nextStep = () => {
    if (currentStep === extrasStepIndex) {
        if (activeExtraTab === 'baggage') { setActiveExtraTab('meal'); window.scrollTo({ top: 0, behavior: 'smooth' }); return; }
        if (activeExtraTab === 'meal') { setActiveExtraTab('seat'); window.scrollTo({ top: 0, behavior: 'smooth' }); return; }
  ...
  const prevStep = () => {
    if (currentStep === 1) {
        onBack();
    } else if (currentStep === extrasStepIndex) {
        if (activeExtraTab === 'seat') { setActiveExtraTab('meal'); window.scrollTo({ top: 0, behavior: 'smooth' }); return; }
        if (activeExtraTab === 'meal') { setActiveExtraTab('baggage'); window.scrollTo({ top: 0, behavior: 'smooth' }); return; }
        setCurrentStep(1);
        window.scrollTo({ top: 0, behavior: 'smooth' });
    } else {
  ...
  useEffect(() => {
    let timerId;
    if (bookingType && currentStep !== paymentStepIndex && bookingTimer > 0) {
      timerId = setTimeout(() => setBookingTimer(prev => prev - 1), 1000);
    } else if (bookingType && currentStep === paymentStepIndex && paymentTimer > 0) {
      timerId = setTimeout(() => setPaymentTimer(prev => prev - 1), 1000);
    }
    return () => clearTimeout(timerId);
  }, [currentStep, bookingTimer, paymentTimer, bookingType, paymentStepIndex]);
      ...
      <div className="bg-white rounded-xl p-4 md:p-5 border border-slate-200">
        ...
        {bookingType === 'umrah' && (
          <div className="border-t border-slate-100 pt-4 mt-2">
            <input type="file" id="passport-upload" className="hidden" accept="image/png, image/jpeg, image/jpg" onChange={handleFileUpload} ref={fileInputRef}/>
  ...
  // --- STEP 2: EXTRAS VIEW (Flight Only) - SUB-FLOW INCLUDED ---
  const renderExtrasStep = () => {
    const subTabs = [
      { id: 'baggage', label: 'Baggage', icon: Luggage },
      { id: 'meal', label: 'Meals', icon: Coffee },
      { id: 'seat', label: 'Seats', icon: User } 
    ];

    return (
      <div className="animate-in fade-in duration-300">
        <div className="mb-4">
          <h2 className="text-2xl font-black text-slate-900 tracking-tight leading-none">Add Extras</h2>
          <p className="text-slate-500 font-medium text-[13px] sm:text-[12px] mt-1.5">Customize your journey with seats, extra baggage, and meals.</p>
        </div>

        {/* Sub-tabs Navigation */}
        <div className="flex gap-2 p-1.5 bg-white border border-slate-200 rounded-xl mb-6 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none]">
          {subTabs.map((tab) => {
            const Icon = tab.icon;
            ...
            return (
              <button
                ...
                className={`flex-1 min-w-[100px] flex items-center justify-center gap-2 py-2.5 rounded-lg text-[12px] font-bold uppercase tracking-widest transition-all ${
                  isActive ? 'bg-[#1a4484] text-white shadow-md' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'
                }`}
              >
        ...
        {/* --- BAGGAGE SUB-VIEW --- */}
        {activeExtraTab === 'baggage' && (
          <div className="bg-white rounded-xl border border-slate-200 p-4 md:p-6 animate-in fade-in slide-in-from-right-4 duration-300">
             <h3 className="font-bold text-slate-800 text-[14px] mb-4 flex items-center gap-2">
               <div className="w-8 h-8 rounded-full bg-blue-50 flex items-center justify-center text-[#1a4484]"><Luggage size={14} /></div>
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
               {BAGGAGE_OPTIONS.map(opt => (
                  <div 
                     key={opt.id} 
                     onClick={() => {
                       setExtras({...extras, baggage: opt});
                       setActiveExtraTab('meal');
                       window.scrollTo({ top: 0, behavior: 'smooth' });
                     }}
                     className={`border rounded-xl p-4 cursor-pointer transition-all flex flex-col justify-between h-full ${extras.baggage.id === opt.id ? 'border-[#1a4484] bg-blue-50/40 ring-1 ring-[#1a4484]' : 'border-slate-200 hover:border-slate-300 hover:bg-slate-50/50'}`}
                  >
                     <div>
                       <div className="flex justify-between items-start mb-2">
                          <span className="text-[14px] font-black text-slate-800">{opt.label}</span>
                          <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${extras.baggage.id === opt.id ? 'border-[#1a4484]' : 'border-slate-300'}`}>
                             {extras.baggage.id === opt.id && <div className="w-2.5 h-2.5 rounded-full bg-[#1a4484]"></div>}
                          </div>
                       </div>
                       <p className="text-[11px] text-slate-500 font-medium mb-3">{opt.desc}</p>
                     </div>
                     <span className="text-[12px] font-black text-[#1a4484] tracking-widest uppercase mt-auto border-t border-slate-100 pt-3">
                        {opt.price === 0 ? 'Included (Free)' : `+ PKR ${opt.price.toLocaleString()}`}
                     </span>
                  </div>
               ))}
             </div>
             <div className="mt-6 flex justify-end">
               <button 
                 onClick={() => {
                   setExtras({...extras, baggage: BAGGAGE_OPTIONS[0]});
                   setActiveExtraTab('meal');
                   window.scrollTo({ top: 0, behavior: 'smooth' });
                 }}
                 className="flex items-center gap-2 text-[12px] font-bold text-slate-500 hover:text-[#1a4484] transition-colors uppercase tracking-widest bg-slate-50 hover:bg-slate-100 py-2 px-4 rounded-lg"
               >
        ...
        {activeExtraTab === 'meal' && (
          <div className="bg-white rounded-xl border border-slate-200 p-4 md:p-6 animate-in fade-in slide-in-from-right-4 duration-300">
             <h3 className="font-bold text-slate-800 text-[14px] mb-4 flex items-center gap-2">
               <div className="w-8 h-8 rounded-full bg-blue-50 flex items-center justify-center text-[#1a4484]"><Coffee size={14} /></div>
               In-Flight Meals
             </h3>
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
               {MEAL_OPTIONS.map(opt => (
                  <div 
                     key={opt.id} 
                     onClick={() => {
                       setExtras({...extras, meal: opt});
                       setActiveExtraTab('seat');
                       window.scrollTo({ top: 0, behavior: 'smooth' });
                     }}
                     className={`border rounded-xl p-4 cursor-pointer transition-all flex flex-col justify-between h-full ${extras.meal.id === opt.id ? 'border-[#1a4484] bg-blue-50/40 ring-1 ring-[#1a4484]' : 'border-slate-200 hover:border-slate-300 hover:bg-slate-50/50'}`}
                  >
                     <div>
                       <div className="flex justify-between items-start mb-2">
                          <span className="text-[14px] font-black text-slate-800">{opt.label}</span>
                          <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${extras.meal.id === opt.id ? 'border-[#1a4484]' : 'border-slate-300'}`}>
                             {extras.meal.id === opt.id && <div className="w-2.5 h-2.5 rounded-full bg-[#1a4484]"></div>}
                          </div>
                       </div>
                       <p className="text-[11px] text-slate-500 font-medium mb-3">{opt.desc}</p>
                     </div>
             ...
             <div className="mt-6 flex justify-end">
               <button 
                 onClick={() => {
                   setExtras({...extras, meal: MEAL_OPTIONS[0]});
                   setActiveExtraTab('seat');
                   window.scrollTo({ top: 0, behavior: 'smooth' });
                 }}
                 className="flex items-center gap-2 text-[12px] font-bold text-slate-500 hover:text-[#1a4484] transition-colors uppercase tracking-widest bg-slate-50 hover:bg-slate-100 py-2 px-4 rounded-lg"
        ...
        {/* --- SEATS SUB-VIEW --- */}
        {activeExtraTab === 'seat' && (
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden flex flex-col md:flex-row animate-in fade-in slide-in-from-right-4 duration-300">
             <div className="w-full md:w-[320px] p-5 border-b md:border-b-0 md:border-r border-slate-200 bg-white">
                <div className="mb-6">
                  <h3 className="font-bold text-[#1a4484] text-[13px] tracking-wide mb-1 underline underline-offset-4 decoration-2 decoration-[#1a4484]">
                    {FLIGHT_DETAILS.depart.code} – {FLIGHT_DETAILS.arrive.code}
                  </h3>
                  <p className="text-[14px] font-black text-slate-800 mt-3">Flights from {FLIGHT_DETAILS.depart.city} to {FLIGHT_DETAILS.arrive.city}</p>
                  <p className="text-[12px] text-slate-500 font-medium mt-1">{FLIGHT_DETAILS.airline} | {FLIGHT_DETAILS.flightNo} | {FLIGHT_DETAILS.class}</p>
                </div>

                <div className="border border-slate-200 rounded-lg p-4 mb-4 bg-white shadow-sm">
                  <h4 className="text-[13px] font-black text-slate-800 mb-4">Seatmap Legend</h4>
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                       <div className="w-[22px] h-[22px] rounded border-2 border-slate-300 bg-slate-200 relative">
                         <div className="absolute top-1 -left-1 w-1 h-3 rounded-full bg-slate-300"></div>
                         <div className="absolute top-1 -right-1 w-1 h-3 rounded-full bg-slate-300"></div>
                       </div>
                       <span className="text-[12px] font-bold text-slate-600">Available</span>
                    </div>
                    <div className="flex items-center gap-3">
                       <div className="w-[22px] h-[22px] rounded border-2 border-slate-700 bg-slate-700 relative">
                         <div className="absolute top-1 -left-1 w-1 h-3 rounded-full bg-slate-700"></div>
                         <div className="absolute top-1 -right-1 w-1 h-3 rounded-full bg-slate-700"></div>
                       </div>
                       <span className="text-[12px] font-bold text-slate-600">Occupied</span>
                    </div>
                    <div className="flex items-center gap-3">
                       <div className="w-[22px] h-[22px] rounded border-2 border-[#dc2626] bg-[#dc2626] relative">
                         <div className="absolute top-1 -left-1 w-1 h-3 rounded-full bg-[#dc2626]"></div>
                         <div className="absolute top-1 -right-1 w-1 h-3 rounded-full bg-[#dc2626]"></div>
                       </div>
                       <span className="text-[12px] font-bold text-slate-600">Selected</span>
                    </div>
                  </div>
                </div>

                <div className="border border-slate-200 rounded-lg p-4 bg-white shadow-sm">
                  <h4 className="text-[13px] font-black text-slate-800 mb-3">Passenger Seat Details</h4>
                  <div className="flex justify-between items-center bg-slate-50 p-2.5 rounded border border-slate-100">
                    <span className="text-[12px] font-bold text-slate-700">{traveler.title} {traveler.givenName || 'Ahsan'} {traveler.surname || 'Raza'}</span>
                    <span className="text-[12px] font-black text-[#1a4484]">
                      {extras.seat.id !== 'none' ? extras.seat.label : 'Not selected'}
                    </span>
                  </div>
                </div>
             </div>

             <div className="flex-1 bg-slate-50/50 p-6 flex flex-col items-center justify-center">
                <div className="relative w-full max-w-[340px]">
                   <div className="h-32 bg-white border border-slate-200 rounded-t-[160px] relative overflow-hidden shadow-sm flex justify-center items-end pb-4 border-b-0">
                      <div className="absolute top-6 flex gap-3 opacity-20">
                         <div className="w-12 h-10 bg-slate-800 rounded-tl-[40px] rounded-br-md transform -skew-x-12"></div>
                         <div className="w-12 h-10 bg-slate-800 rounded-tr-[40px] rounded-bl-md transform skew-x-12"></div>
                      </div>
                   ...
                   <div className="bg-white border border-slate-200 px-4 sm:px-6 py-6 shadow-sm min-h-[400px] max-h-[500px] overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                      <div className="flex justify-between items-center mb-6">
                         <span className="text-[#dc2626] text-[9px] font-black tracking-widest uppercase flex items-center gap-1"><ArrowLeft size={10} strokeWidth={3}/> Exit</span>
                         <span className="text-[#dc2626] text-[9px] font-black tracking-widest uppercase flex items-center gap-1">Exit <ArrowRight size={10} strokeWidth={3}/></span>
                      </div>
                      ...
                      {seatMap.map((row, rIdx) => (
                         <React.Fragment key={row.rowNum}>
                           <div className="flex justify-between items-center mb-2.5">
                             <div className="flex gap-1.5 sm:gap-2">
                               {row.seats.slice(0, 3).map(seat => {
                                 const isSelected = extras.seat.id === seat.id;
                                 const status = seat.isOccupied ? 'occupied' : isSelected ? 'selected' : 'available';
                                 
                                 let colorClass = "bg-slate-200 border-slate-300";
                                 let armColor = "bg-slate-300";
                                 if (status === 'occupied') {
                                    colorClass = "bg-slate-700 border-slate-700 cursor-not-allowed";
                                    armColor = "bg-slate-700";
                                 } else if (status === 'selected') {
                                    colorClass = "bg-[#dc2626] border-[#dc2626]";
                                    armColor = "bg-[#dc2626]";
                                 } else {
                                    colorClass = "bg-[#e2e8f0] border-slate-300 hover:bg-slate-300 cursor-pointer";
                                 }
                                 ...
                                 return (
                                   <div 
                                      ...
                                      onClick={() => {
                                        if (status !== 'occupied') {
                                          setExtras({...extras, seat: { id: seat.id, label: seat.id, price: seat.price }});
                                        }
                                      }}
                                      className={`w-[26px] h-[30px] rounded border-2 relative transition-colors group ${colorClass}`}
                                   >
                                      <div className={`absolute top-1 -left-[3px] w-[3px] h-4 rounded-full ${armColor}`}></div>
                                      <div className={`absolute top-1 -right-[3px] w-[3px] h-4 rounded-full ${armColor}`}></div>
                                      
                                      {status !== 'occupied' && (
                                        <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-[10px] py-1.5 px-2.5 rounded opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-50 shadow-lg flex flex-col items-center gap-1 transition-opacity">
                                          <span className="font-medium">PKR {seat.price.toLocaleString()}</span>
                                          <span className="bg-[#dc2626] text-white text-[9px] font-black px-1.5 rounded">{seat.id}</span>
                                        </div>
                                      )}
                                   </div>
                                 );
                             ...
                             <div className="w-6 text-center text-[11px] font-black text-slate-400 select-none">
                                {row.rowNum}
                             </div>
                           ...
                           {(row.rowNum === 11 || row.rowNum === 12) && (
                              <div className="flex justify-between items-center my-4 opacity-50">
                                 <span className="text-[#dc2626] text-[9px] font-black tracking-widest uppercase flex items-center gap-1"><ArrowLeft size={10} strokeWidth={3}/> Exit</span>
                                 <span className="text-[#dc2626] text-[9px] font-black tracking-widest uppercase flex items-center gap-1">Exit <ArrowRight size={10} strokeWidth={3}/></span>
                              </div>
                           )}
                ...
                <div className="w-full max-w-[340px] mt-6 flex justify-end">
                  <button 
                    onClick={() => {
                      setCurrentStep(reviewStepIndex);
                      window.scrollTo({ top: 0, behavior: 'smooth' });
                    }}
                    className="flex items-center gap-2 text-[12px] font-bold text-slate-500 hover:text-[#1a4484] transition-colors uppercase tracking-widest bg-white hover:bg-slate-100 py-2 px-4 rounded-lg shadow-sm border border-slate-200"
                  >
                    Skip <ArrowRight size={14} />
                  </button>
                </div>
        ...
        {paymentMethod === '1bill' && (
          <div className="bg-white rounded-xl p-4 md:p-5 border border-slate-200 animate-in fade-in">
             ...
             <ol className="space-y-3 text-[12px] text-slate-600 font-medium">
               ...
               <li className="flex gap-2"><span>2-</span> Present the bill to the teller and tell him that it is a 1Bill Invoice payment.</li>
  ...
  return (
    <div className="bg-[#fafcff] min-h-screen font-sans pb-28 selection:bg-blue-100 relative animate-in fade-in slide-in-from-bottom-2 duration-700">
      <style dangerouslySetInnerHTML={{ __html: "@import url('https://fonts.googleapis.com/css2?family=Montserrat:wght@500;600;700;800;900&family=Eczar:wght@600;700;800&display=swap'); body { font-family: 'Montserrat', sans-serif; }" }} />
      ...
      {bookingType && (
        <div className="sticky top-0 z-50 flex flex-col w-full shadow-sm">
          ...
          <Stepper currentStep={currentStep} bookingType={bookingType} onBack={() => {
            ...
            else if(currentStep !== successStepIndex) prevStep();
            else window.location.reload();
          }} />

          {/* Global Top Timer Banner (Only shown in steps before success) */}
          {currentStep !== successStepIndex && (
            <div className={`w-full ${currentStep === paymentStepIndex ? 'bg-orange-50 border-orange-100' : 'bg-blue-50/50 border-blue-100'} border-b flex items-center justify-center p-2.5 gap-2 relative transition-colors`}>
              <Clock size={14} className={`${currentStep === paymentStepIndex ? 'text-orange-500' : 'text-[#1a4484]'} animate-pulse`} />
              <span className="text-[11px] font-bold text-slate-700 uppercase tracking-widest">
                {currentStep === paymentStepIndex ? "Complete payment in: " : "Time remaining to complete booking: "}
                <span className={`${currentStep === paymentStepIndex ? 'text-orange-600' : 'text-[#1a4484]'} font-black`}>
                  {formatTime(currentStep === paymentStepIndex ? paymentTimer : bookingTimer)}
                </span>
              </span>
            </div>
          )}
        ...
        {bookingType && (
          <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 items-start">
            ...
            <div className="w-full lg:w-[65%]">
              ...
              {bookingType === 'ticket' && currentStep === extrasStepIndex && renderExtrasStep()}
              {currentStep === reviewStepIndex && renderReviewStep()}
              {currentStep === paymentStepIndex && renderPaymentStep()}
              {currentStep === successStepIndex && renderSuccessStep()}
            </div>

            {/* Right Content Area (Price Summary - Hidden on Success) */}
            {currentStep !== successStepIndex && (
              <div className="w-full lg:w-[35%] lg:sticky lg:top-[120px]">
                 <div className="bg-white border border-slate-200 rounded-xl shadow-sm flex flex-col">
                   <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 rounded-t-xl">
                     ...
                     <span className="text-slate-500 text-[10px] sm:text-[9px] font-bold uppercase tracking-widest border border-slate-200 bg-white px-1.5 py-0.5 rounded shadow-sm">{totalTravelers} {totalTravelers > 1 ? 'Travelers' : 'Adult'}</span>
                   </div>

                   {bookingType === 'ticket' && (
                     <div className="p-4 border-b border-slate-100 bg-white">
                        <div className="flex items-center gap-3">
                           <div className="w-10 h-10 border border-slate-100 rounded flex items-center justify-center bg-white overflow-hidden p-1">
                              <img src={FLIGHT_DETAILS.logo} alt={FLIGHT_DETAILS.airline} className="max-w-full max-h-full object-contain" />
                           </div>
                           <div className="flex-1">
                              <h4 className="text-[12px] font-black text-slate-800">{FLIGHT_DETAILS.depart.city} ({FLIGHT_DETAILS.depart.code}) to {FLIGHT_DETAILS.arrive.city} ({FLIGHT_DETAILS.arrive.code})</h4>
                              <p className="text-[10px] font-bold text-slate-500">{FLIGHT_DETAILS.airline} • {FLIGHT_DETAILS.flightNo} • {FLIGHT_DETAILS.class}</p>
                           </div>
                        </div>
                     </div>
                   )}
                   ...
                   <div className="p-4 space-y-4 flex-1 bg-white">
                     <div className="flex justify-between items-center">
                       <div className="flex flex-col">
                         <span className="text-[12px] sm:text-[11px] font-bold text-slate-500">Base Fare ({totalTravelers} PAX)</span>
                         <span className="text-[9.5px] font-semibold text-slate-400 mt-0.5">Refundable (Subject to penalty)</span>
                       </div>
                     ...
                     <div className="flex justify-between items-center">
                       <div className="flex flex-col">
                         ...
                         <span className="text-[9.5px] font-semibold text-slate-400 mt-0.5">Fully Refundable</span>
                       </div>
                       <span className="text-[13px] sm:text-[12px] font-black text-slate-800">PKR {PRICE_DETAILS.taxes.toLocaleString()}</span>
                     </div>
                     <div className="flex justify-between items-center">
                       <div className="flex flex-col">
                         <span className="text-[12px] sm:text-[11px] font-bold text-slate-500">Booking Fee</span>
                         <span className="text-[9.5px] font-semibold text-slate-400 mt-0.5">Non-Refundable</span>
                       </div>
                       <span className="text-[13px] sm:text-[12px] font-black text-slate-800">PKR {PRICE_DETAILS.bookingFee.toLocaleString()}</span>
                     </div>
                     {PRICE_DETAILS.paymentFee > 0 && (
                       <div className="flex justify-between items-center text-slate-500">
                         <div className="flex flex-col">
                           <span className="text-[12px] sm:text-[11px] font-bold">Payment Gateway Fee</span>
                           <span className="text-[9.5px] font-semibold text-slate-400 mt-0.5">Non-Refundable</span>
                         </div>
                         <span className="text-[13px] sm:text-[12px] font-black text-slate-800">PKR {PRICE_DETAILS.paymentFee.toLocaleString()}</span>
                       </div>
                     )}

                     {bookingType === 'ticket' && (extras.baggage.id !== 'none' || extras.meal.id !== 'none' || extras.seat.id !== 'none') && (
                       <div className="pt-2 mt-1 border-t border-slate-100 flex flex-col gap-2 relative">
                         <div className="flex justify-between items-center text-[#1a4484] mb-1">
                           <div className="flex flex-col">
                             <span className="text-[12px] sm:text-[11px] font-bold">Add-ons (Extras)</span>
                             <span className="text-[9.5px] font-semibold text-slate-400 mt-0.5">Non-Refundable</span>
                           </div>
                           <span className="text-[13px] sm:text-[12px] font-black">PKR {extrasTotal.toLocaleString()}</span>
                         </div>
                         
                         {extras.baggage.id !== 'none' && (
                           <div className="flex justify-between items-center text-slate-600 pl-2 border-l-2 border-[#1a4484]/30">
                             <span className="text-[11px] sm:text-[10px] font-semibold">{extras.baggage.label} (Baggage)</span>
                             <span className="text-[11px] sm:text-[10px] font-bold">{extras.baggage.price > 0 ? `PKR ${extras.baggage.price.toLocaleString()}` : 'Free'}</span>
                           </div>
                         )}

                         {extras.meal.id !== 'none' && (
                           <div className="flex justify-between items-center text-slate-600 pl-2 border-l-2 border-[#1a4484]/30">
                             <span className="text-[11px] sm:text-[10px] font-semibold">{extras.meal.label}</span>
                             <span className="text-[11px] sm:text-[10px] font-bold">{extras.meal.price > 0 ? `PKR ${extras.meal.price.toLocaleString()}` : 'Free'}</span>
                           </div>
                         )}

                         {extras.seat.id !== 'none' && (
                           <div className="flex justify-between items-center text-slate-600 pl-2 border-l-2 border-[#1a4484]/30">
                             <span className="text-[11px] sm:text-[10px] font-semibold">Seat {extras.seat.label}</span>
                             <span className="text-[11px] sm:text-[10px] font-bold">{extras.seat.price > 0 ? `PKR ${extras.seat.price.toLocaleString()}` : 'Free'}</span>
                           </div>
                         )}
                       </div>
                     )}
                     ...
                     <div className="border-t border-slate-100 pt-3 mt-1">
                       <div className="flex justify-between items-end mb-2 gap-2">
                         <span className="text-[10px] sm:text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5 min-w-[100px]">Total Flight/Package</span>
                         <span className="text-[14px] font-black text-slate-800 tracking-tight leading-none text-right whitespace-nowrap" style={{ fontFamily: "'Montserrat', sans-serif" }}>
                           PKR {PRICE_DETAILS.total.toLocaleString()}
                         </span>
                       </div>

                       {/* Show Partial Payment Breakdown ONLY if split allowed */}
                       {currentStep === paymentStepIndex && bookingType === 'umrah' && ['bank', 'wallet', 'atm', '1bill'].includes(paymentMethod) && paymentAmount !== PRICE_DETAILS.total && (
                       ...
                       {!(currentStep === paymentStepIndex && bookingType === 'umrah' && ['bank', 'wallet', 'atm', '1bill'].includes(paymentMethod) && paymentAmount !== PRICE_DETAILS.total) && (
                         <div className="flex justify-between items-center mt-3 gap-3">
                           <span className="text-[10px] sm:text-[9px] font-black text-[#1a4484] uppercase tracking-widest min-w-[80px]">Total Payable Now</span>
                           <span className="text-xl sm:text-2xl font-black text-[#1a4484] tracking-tight leading-none text-right whitespace-nowrap" style={{ fontFamily: "'Montserrat', sans-serif" }}>
                             PKR {PRICE_DETAILS.total.toLocaleString()}
                           </span>
                         </div>
                       )}
                   ...
                   <div className="hidden lg:block p-4 pt-0 mt-1">
                     ...
                     >
                       {currentStep === 1 
                          ? (bookingType === 'ticket' ? 'Continue to Extras' : 'Continue to Review') 
                          : currentStep === extrasStepIndex 
                          ? 'Continue to Review' 
                          : currentStep === reviewStepIndex 
                          ? 'Make Booking' 
                          : (paymentMethod === 'card' ? 'Pay Now' : 'Verify Payment Now')}
      ...
      {bookingType && currentStep !== successStepIndex && (
        <div className="lg:hidden fixed bottom-0 left-0 w-full bg-white/95 backdrop-blur-md border-t border-slate-200 p-4 z-30 flex items-center justify-between shadow-[0_-10px_30px_rgba(0,0,0,0.05)]">
           <div className="flex flex-col">
             ...
             <span className="text-xl sm:text-lg font-black text-[#1a4484] leading-none" style={{ fontFamily: "'Montserrat', sans-serif" }}>
               PKR {currentStep === paymentStepIndex && ['bank', 'wallet', 'atm', '1bill'].includes(paymentMethod) && bookingType === 'umrah' ? paymentAmount.toLocaleString() : PRICE_DETAILS.total.toLocaleString()}
             </span>
           </div>
           <button 
             onClick={nextStep}
           ...
           >
             {currentStep === paymentStepIndex ? (paymentMethod === 'card' ? 'Pay Now' : 'Verify Payment Now') 
              : currentStep === reviewStepIndex ? 'Make Booking' 
